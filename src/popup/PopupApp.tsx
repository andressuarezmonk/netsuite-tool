...
export default function PopupApp() {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      if (url.includes(".netsuite.com")) {
        try {
          ...
          setNsUrl(
            `${origin}/app/site/hosting/scriptlet.nl?script=2375&deploy=1`,
          );
          return;
        } catch {
          /* ignore */
      ...
      // Fallback to last stored NS URL
      chrome.storage.local.get("lastNsUrl", (result) => {
        if (result.lastNsUrl) setNsUrl(result.lastNsUrl as string);
      });
    });
  }, []); // runs once on mount — nsUrl is set via setNsUrl inside
  ...
  return (
    <div
      style={{ width: 240, padding: 16, fontFamily: "system-ui, sans-serif" }}
    >
