...
import { DAYS, DayKey } from '@/lib/constants';
...
interface Props {
  ...
  weekISO: string;
  onSave: (row: TimeRow, dayKey: DayKey, hours: number, memo: string) => Promise<void>;
}

export default function WeekGrid({ weekData, weekISO, onSave }: Props) {
      ...
      <table className="ft-table">
        <thead>
          <tr>
            ...
            {DAYS.map((_, i) => (
              <th key={DAYS[i]} className={`ft-col-day${dayDates[i] === today ? ' ft-today' : ''}`}>
                {DAY_LABELS[i]}
...
function TimeRowComponent({ row, dayDates, today, onSave }: RowProps) {
  const rowTotal = DAYS.reduce((s, dk) => s + (row.days[dk]?.hours ?? 0), 0);

  return (
    <tr>
      <td className="ft-col-proj" title={row.projId}>{row.projName}</td>
      <td className="ft-col-task" title={row.taskId}>{row.taskName || '—'}</td>
      {DAYS.map((dk, i) => (
        <DayCell
          key={dk}
          row={row}
...
function DayCell({ row, dayKey, isToday, onSave }: CellProps) {
  ...
  const [value, setValue] = useState(entry?.hours ? formatHours(entry.hours) : '');
  const [saving, setSaving] = useState(false);
  const prevRef = useRef(value);

  // Sync when data reloads externally (week nav)
...
function DayTotals({ rows, dayDates, today }: { rows: TimeRow[]; dayDates: string[]; today: string }) {
  ...
  return (
    <tr>
      <td colSpan={2} className="ft-total-label">Week total</td>
      {DAYS.map((dk, i) => {
        const total = rows.reduce((s, r) => s + (r.days[dk]?.hours ?? 0), 0);
        return (
          <td key={dk} className={`ft-col-day${dayDates[i] === today ? ' ft-today' : ''}`}>
            {total ? formatHours(total) : ''}
          </td>
        );
      })}
      <td className="ft-col-total">{grandTotal ? formatHours(grandTotal) : ''}</td>
    </tr>
  );
}
